...
std::vector<std::pair<double, int>> Information;
//map vector code
...
void delete_index(int id, int vector_code){
    ...
    if(decr_vector[vector_code] == "computer"){
        comp_pool.erase( remove(comp_pool.begin(), comp_pool.end(), comp_pool[id]), comp_pool.end());
    }else{
        human_pool.erase( remove(human_pool.begin(), human_pool.end(), human_pool[id]), human_pool.end());
    }
}
/*
...
bool compare_info(std::pair<double, int> a, std::pair<double, int> b){
    if(a.first == b.first){
        //if information is the same, just say yes
        return true;
    }
...
int calculate_best_guess(){
    ...
    for(int i = 0;i<num_size;i++){
        ...
        //reset
        for(int l = 0;l<9;l++){
            case_counter[valid_case[l]]=0;
        }
        //get all case distribution 
        for(int j = 0;j<num_size;j++){
            case_counter[get_case(comp_pool[i], comp_pool[j], NO_THRES, NO_THRES, false)]+=1.0;
        }
    ...
    //well all of this is just random, because informatin are all the same
    for(int i = 0;i<total;i++){
        std::cout << "[" <<Information[num_size-i-1].second<< "]:" << comp_pool[Information[num_size-i-1].second] << "\n"; ;
    }
    int idMax = Information.back().second;
    std::cout<<"--------------\n";
    return idMax;
}
