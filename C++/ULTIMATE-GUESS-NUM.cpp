...
std::vector<std::string> universe;//all possible guesses
//bool map check if a guess is still valid
std::map<std::string, bool> valid_guess;
//map vector code
...
void delete_index(int id, int vector_code){
    ...
    if(decr_vector[vector_code] == "computer"){
        valid_guess[comp_pool[id]] = false;
...
int calculate_best_guess(){
    ...
    for(int i = 0;i<720;i++){
        ...
        for(int j = 0;j<comp_pool.size();j++){
            case_counter[get_case(universe[i], comp_pool[j], NO_THRES, NO_THRES, false)]+=1.0;
        }
        //count information of each valid case
        for(int k = 0;k<9;k++){
            sum_info += count_information(case_counter[valid_case[k]]/(720.0));
        }
        if(sum_info==maxInfo){
            idMax = valid_guess[universe[i]] ? i:idMax;
            continue;
        }
        if(sum_info > maxInfo){
            idMax = i;
...
void computer_algorithm(std::string comp_secret, bool& game_over){
    ...
    if(comp_pool.size()<3){
        comp_guess = comp_pool[0];
    }else{
        index_guess = calculate_best_guess();
        comp_guess = universe[index_guess];
    }
    std::cout << "Is this your number? "<< comp_guess << "\n";
    std::cin >> angka >> pos;
    cases = 3*angka + pos;
    if(cases != 12){
        eliminate(cases, comp_guess, enc_vector["computer"]);
    }else{
        std::cout << "HELL YEAH\n";
...
float calc_prob_comp_win(int whos_turn){
    if(comp_pool.size()==1 && whos_turn%2==1){
        return 100.0;
    }
...
int main(){
	...
	int turns = 1;
    char ans;
    std::cout << "do you want me to go first? [y/n]\n";
    std::cin >> ans;
    turns -= ans=='n' || ans=='N';
	...
	while(!game_over){
        ...
        std::cout<<"Probability Computer Wins: " << calc_prob_comp_win(turns) << "%\n";
		//comp turn
		if(turns % 2 == 1){
