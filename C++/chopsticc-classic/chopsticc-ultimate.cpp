...
int evalute_game_pos(uint8_t game_pos[4], bool maximizingPlayer);
void reset_game_pos(uint8_t game_pos[4], uint8_t reset_pos[4]);
int minimax(uint8_t game_pos[4], int depth, int alpha, int beta, bool maximizingPlayer);
...
void computer_turn(uint8_t game_pos[4]){
    ...
    int depth = 10;//thinking 10 moves ahead
    ...
    for(auto move : availableMove){
        ...
        int alpha = NEG_INF;
        int beta = INF;
        int move_score = minimax(copy_of_game_pos, depth, alpha, beta, false);
...
int evalute_game_pos(uint8_t game_pos[4], char winner, bool maximizingPlayer){
    if(winner != 'N'){
        return WIN_CONDITION[winner];
    }
    //so none of em is winner huh
    int score = game_pos[0]*5 + game_pos[1];//score 2 tangan is better
    score -= (game_pos[2]*5 + game_pos[3]);//score lawan
    if(maximizingPlayer){
...
int minimax(uint8_t game_pos[4], int depth, int alpha, int beta, bool maximizingPlayer){
    ...
    */
    ...
    int turn = 0;
    turn += maximizingPlayer;
    char winner = check_for_win(game_pos, turn);
    if(depth==0 || winner != 'N'){
        return evalute_game_pos(game_pos, winner, maximizingPlayer);
    }
    if(maximizingPlayer){
        //computer's turn
        int value = NEG_INF;
        ...
        for(auto move: available_compMove){
            ...
            value = max(value, minimax(game_pos, depth-1, alpha, beta,false));
    ...
    for(auto move : available_humanMove){
        do_move(move, game_pos);
        value = min(value, minimax(game_pos, depth-1, alpha, beta, true));
        beta = min(beta, value);
        reset_game_pos(game_pos, temp_pos);
        if(value <= alpha){
            break;
